# method 2, math, time O(str(N)), space O(1)
class Solution(object):
    def atMostNGivenDigitSet(self, D, N):
        ...
        :rtype: int
        """
        s_N = str(N)  # string of N
        cnt = 0

        # count numbers with length < len(s_N)
        for m in range(1, len(s_N)):
            cnt += len(D) ** m

        # # count numbers with length == len(s_N)
        for i, digit in enumerate(s_N):
            j = bisect.bisect_left(D, digit)
            cnt += j * len(D) ** (len(s_N) - i - 1)
            if j == len(D) or D[j] != digit:
                break
        else:
            if s_N[-1] in D:
                cnt += 1
...
# method 1, brute force, time O(res), space O(len(str(N)) due to recursion
...
class Solution1(object):
    def atMostNGivenDigitSet(self, D, N):
        ...
        for digit in digits:
            if digit <= N:
                self.dfs(digit, digits, N)
        return self.cnt

    def dfs(self, num, digits, N):
        self.cnt += 1
        for digit in digits:
            new_num = num * 10 + digit
            if new_num <= N:
                self.dfs(num * 10 + digit, digits, N)
