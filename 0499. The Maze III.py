    ...
    def findShortestWay(self, maze, ball, hole):
        ...
        while queue:
            ...
            if p < 0 or p >= m or q < 0 or q >= n or maze[p][q] == 1:  # change direction
                for k in range(4):
                    # if visited[i][j]&(1<<k) == 0: continue   # adding this sentence is a mistake !!!
                    # nodes in same level might be affecting each other
                    visited[i][j] |= 1 << k
                    x, y = i + dirs[k][0], j + dirs[k][1]
                    if 0 <= x < m and 0 <= y < n and maze[x][y] == 0 and visited[x][y] & (1 << k) == 0:
                        visited[x][y] |= 1 << k
                        queue.append((x, y, path + dirs[k][2], k))
            else:  # continue walk
...
by moving the shortest distance. The distance is defined by the number of empty spaces traveled 
by the ball from the start position (excluded) to the hole (included). Output the moving directions 
by using 'u', 'd', 'l' and 'r'. Since there could be several different shortest ways, 
...
You may assume that the borders of the maze are all walls. The ball and the hole coordinates are represented 
