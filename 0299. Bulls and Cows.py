# time O(n), space O(m), m = 10, n = len(secret)
...
class Solution(object):
    def getHint(self, secret, guess):
        ...
        for c in secret_count:
            if c in guess_count:
                cow += min(secret_count[c], guess_count[c])
...
but locate in the wrong position (called "cows"). Your friend will use successive guesses and 
