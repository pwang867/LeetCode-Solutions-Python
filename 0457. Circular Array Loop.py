# time O(n), space O(1), a circle will definitely result in cnt > len(nums)
...
end at the same index and the cycle's length > 1. Furthermore, movements 
in a cycle must all follow a single direction. In other words, 
