...
class Solution(object):
    def findOrder(self, numCourses, prerequisites):
        ...
        def visit(i):
            """ 
            DFS method
            visit all vertice directed from vertex i
            change the marks, and update ans
            return True if no cycle, else return False
            """
            if mark[i] == 1:
                return True
            if mark[i] == -1:
                return False
            mark[i] = -1  # temporary mark, must have !!!
            for v in d[i]:
                if not visit(v):
                    return False
            mark[i] = 1
            ans.append(i)  # only add this one more sentence compared to Course Schedule I
            return True

        for i in range(numCourses):
            if mark[i] == 0:
                if not visit(i):  # when a cycle is found
                    return []
        return list(ans)
