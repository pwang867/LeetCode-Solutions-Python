"""
I did not implement the algorithm but it is actually a polynomial solution.
I copied and pasted the implementation from the post
https://programtalk.com/vs2/python/12776/hungarian-algorithm/hungarian.py/

*Think about Hungarian algorithm as a black box. Your job is to build the matrix, properly padded with 0s (optional) and use the black box. That's it. No guarantee of the correctness of the implementation. Skip the first 416+ lines. *
"""
...
class Solution(object):
	def assignBikes(self, workers, bikes):
			...
			for i in range(m):
				for j in range(n):
					graph[i][j] = abs(workers[i][0] - bikes[j][0]) + abs(workers[i][1] - bikes[j][1])
			#for i in range(n - m):
			#    graph.append([0] * n)
