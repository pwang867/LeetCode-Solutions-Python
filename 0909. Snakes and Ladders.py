# time/space O(n^2)
# trick: whenever we see a ladder, only save the final position to visited
# and only check if the final position of a step is in visited
    ...
    def snakesAndLadders(self, board):
        ...
        while queue:
            for _ in range(len(queue)):
                ...
                for i in range(1, 7):
                    next_pos = pos + i
                    if next_pos <= target:  # can not check visited here
                        val = arr[next_pos - 1]
                        if val != -1:
                            next_pos = val
                        if next_pos not in visited:  # only check the final position
...
left of the board, and alternating direction each row.  For example, for a 6 x 6 board, the numbers 
