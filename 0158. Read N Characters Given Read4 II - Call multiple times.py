...
class Solution(object):
    def __init__(self):
        self.queue = deque([])   # use extra buffer to save the unread chars for next read()
    ...
    def read(self, buf, n):
        ...
        # read the remaining chars from previous read
        ...
        # read from file when we need more chars
        while i < n:
            buf4 = [""]*4
            cnt = read4(buf4)
            if not cnt:  # end of file
                return i
            for j in range(cnt):
                buf[i] = buf4[j]
                i += 1
                if i == n:
                    self.queue.extend(buf4[j+1:])
                    return n
